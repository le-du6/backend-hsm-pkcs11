...
type SessionFn<T> = (session: number) => T | Promise<T>; // Session handle number

class PKCS11Service {
  private static _instance: PKCS11Service;
  private pkcs11: any | null = null;
  private moduleLoaded = false;
  private slot: any | null = null;
  private slotId: number | null = null;
  private initialized = false;
  private tokenLabel?: string;
  private pin?: string;

  static get instance(): PKCS11Service {
    if (!this._instance) this._instance = new PKCS11Service();
  ...
  init(): void {
    ...
    if (!this.tokenLabel) throw internal('PKCS11_TOKEN_LABEL manquant');
    if (!this.pin) throw internal('PKCS11_PIN manquant');
    ...
    try {
      ...
      logger.info(
        { modulePath, slotId: this.slotId, tokenLabel: this.tokenLabel },
        'pkcs11_initialized',
      );
  ...
  private commonModulePaths(): string[] {
  ...
  private discoverHomebrewPaths(): string[] {
    const paths: string[] = [];
    try {
      const base = '/opt/homebrew/Cellar/softhsm';
      if (!fs.existsSync(base)) return paths;
  ...
  private resolveModulePath(): string {
    ...
    for (const p of this.commonModulePaths()) {
      try {
        ...
        let detail: any = { code: e?.code, message: e?.message };
        try {
    ...
    throw internal(
      'PKCS11_MODULE_PATH introuvable. Essayé: ' +
        tried.join(', ') +
  ...
  private findSlot() {
    ...
    if (slots.length && this.slotId == null) {
      ...
      this.slotId = slots[0];
      this.slot = { slotHandle: slots[0] } as any;
      const info = this.pkcs11.C_GetTokenInfo(slots[0]);
      logger.warn(
        { wantedLabel: this.tokenLabel, usingLabel: info.label.trim() },
        'token_label_mismatch',
      );
    }
    if (this.slotId == null) throw internal('Aucun slot/token disponible');
  }

  async withSession<T>(fn: SessionFn<T>, opts: SessionOptions = {}): Promise<T> {
    if (!this.initialized) this.init();
    if (!this.pkcs11 || this.slotId == null) throw internal('PKCS#11 non initialisé');
    ...
    try {
      ...
      if (session == null) throw internal('Session non ouverte');
  ...
  get pkcs(): any {
    ...
    if (!this.pkcs11) throw internal('PKCS#11 non dispo');
    return this.pkcs11;
  }
  newId(): Buffer {
