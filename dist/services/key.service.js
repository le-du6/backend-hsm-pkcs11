...
function derToSpkiPem(der) {
    const b64 = der.toString('base64');
    const lines = b64.match(/.{1,64}/g) || [];
    return `-----BEGIN PUBLIC KEY-----\n${lines.join('\n')}\n-----END PUBLIC KEY-----`;
}
export class KeyService {
    async createECKeyPair({ label, id, curve = 'P-256' }) {
        if (curve !== 'P-256')
            throw new Error('Curve non supportée');
        const pkcs = pkcs11Service.pkcs;
        const ckaId = id ? Buffer.from(id, 'hex') : pkcs11Service.newId();
        const finalLabel = label || `key-${Date.now().toString(36)}-${ckaId.toString('hex').slice(0, 6)}`;
        return pkcs11Service.withSession((session) => {
            pkcs.C_GenerateKeyPair(session, {
    ...
    findPublicKeyByIdRaw(session, ckaId, attrs) {
        const pkcs = pkcs11Service.pkcs;
        ...
        for (const v of values) {
            if (v.type === pkcs11.CKA_LABEL)
                out.label = v.value.toString();
            if (v.type === pkcs11.CKA_ID)
                out.id = v.value;
            if (v.type === pkcs11.CKA_EC_POINT)
                out.ecPoint = v.value;
        }
        return out;
    }
            ...
            while (objs.length) {
                for (const o of objs) {
                    try {
                        ...
                        const idAttr = attrs.find((a) => a.type === pkcs11.CKA_ID);
                        const labelAttr = attrs.find((a) => a.type === pkcs11.CKA_LABEL);
                        if (idAttr && labelAttr) {
                            results.push({
    ...
    async getKeyById(id) {
        ...
        return pkcs11Service.withSession((session) => {
            ...
            pkcs.C_FindObjectsFinal(session);
            if (!h)
                throw notFound('Clé introuvable');
